# Get personal income from CA1 table for SC counties for all years
import requests
import json
...
with open("ca1_personal_income_sc.json", "w") as jsondata:  # Write json response to file
    jsondata.write(json.dumps(data, sort_keys=True, indent=2))

for item in data["BEAAPI"]["Results"]["Data"]:  # Printing the info
    dollar_unit = item["CL_UNIT"]
    dollars = item["DataValue"]
    ...
    print(year + ": " + location_name + "| " + dollars)